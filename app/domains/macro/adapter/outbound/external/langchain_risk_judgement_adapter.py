...
_BASELINE_INSTRUCTIONS = (
    ...
    "2) 현황판 상단에 이미 Risk-on / Risk-off 상태 배지가 크게 노출되므로, reasons 3줄은 "
    "   결론 문구(예: 'Risk-on 입니다', 'Risk-off 로 판단됩니다', '시장은 Risk-on 쪽에 기울기가 "
    "   있습니다' 등)를 **절대 반복하지 마십시오**. 한 줄은 새로운 실질 정보여야 합니다.\n"
    "3) 첫 번째 문장: 현재 판단을 지지하는 가장 강한 드라이버 하나를 구체적 채널(유동성, 연준 정책 경로, "
    "   실질금리·장단기 스프레드, 달러지수 DXY, 신용 스프레드, 브렉이븐·기대인플레이션, VIX, "
    "   지정학 리스크, 실적 모멘텀, 외국인·기관 수급, 크로스에셋 로테이션 등)에서 기관급 용어로 "
    "   곧바로 설명합니다. 상태 단정 표현으로 시작하지 마십시오.\n"
    "4) 두 번째 문장: 첫 문장을 보완하거나 교차 확인하는 또 다른 드라이버·지표·시그널을 제시합니다.\n"
    "5) 세 번째 문장: 현 판단을 무효화할 수 있는 반증 시그널(invalidation) 또는 포지셔닝/리스크 "
    "   관리 시사점을 간결하게 덧붙입니다.\n"
    "6) 시장 표준 용어(VIX, DXY, 2s10s, ERP, 크레딧 스프레드, ISM, CPI, PCE, WTI, EM FX, "
    "   리스크 온·오프 로테이션 등)를 자연스럽게 섞되 과도한 나열은 금지합니다.\n"
    "7) 감정적 표현, 투자 권유 문구('매수하십시오', '사셔야 합니다' 등)는 금지합니다. "
    "   저작권 소지가 있는 외부 특정 인물·채널·영상·리서치 기관명을 절대 노출하지 마십시오. "
    "   '학습 노트', '참고 영상', '학습 컨텐츠' 같은 내부 용어도 사용하지 마십시오.\n"
    "8) 'Antelligen AI' 라는 브랜드명은 3문장 전체에서 **최대 1회만** 자연스럽게 언급할 수 있습니다. "
    "   자연스럽지 않다면 아예 언급하지 않아도 됩니다. 두 문장 이상에서 반복 언급하는 것은 절대 금지합니다. "
    "   나머지 문장에서는 '매크로 엔진', '크로스에셋 시그널', '거시 데이터' 같은 일반 표현을 사용합니다.\n"
    "9) 자화자찬·홍보성 문구는 피하고, 기관 투자자 브리핑의 객관적 톤을 유지합니다.\n"
    "10) 문장 길이는 간결하게 유지하고, 불필요한 부사와 애매모호한 관용구를 제거합니다.\n"
    "11) 모든 문장은 반드시 한국어 존댓말(격식체)로 마무리합니다. 문장 종결 어미는 "
    "    '~입니다', '~습니다', '~됩니다', '~보입니다', '~판단됩니다' 와 같이 정중한 종결 형태만 "
    "    허용되며, '~이다', '~한다', '~된다' 같은 해라체/평서문 어미는 절대 사용하지 마십시오.\n"
    "12) 숫자 레벨 표기: 모든 가격·지수 레벨·구간 숫자(예: 920, 930, 1000, 3200, 5800)는 "
...
class LangChainRiskJudgementAdapter(RiskJudgementLlmPort):
    ...
    async def _judge_baseline(
        ...
        # baseline 은 JSON schema 를 쓰지 않는 자유 서술이라 reasoning 체인이 길어질 수 있어
        # 강제로 max_output_tokens + reasoning=low 로 제한한다.
        result = await self._client.create(
            instructions=instructions,
